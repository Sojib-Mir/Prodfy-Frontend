...
export default function Login() {
  ...
  const { loginEmailAndPassword, loginGoogle, setLoading, setUser } = useAuth();
  ...
  const handleGooglSignin = () => {
    loginGoogle().then(async (res) => {
      setLoading(false);
      setUser(res.user);

      const token = await res.user.getIdToken(); // 🚨 Cookies.set: path: '/' সেট করা গুরুত্বপূর্ণ, যা আপনি করেছেন।
      Cookies.set("fbToken", token, { path: "/" });
      router.push("/");
